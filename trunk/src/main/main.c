...
void setup_Rmainloop(void)
{
    ...
    R_Toplevel.fnbase = R_BCNodeStackFnBase;
#ifdef BC_INT_STACK
...
static void printBrowserHelp(void)
{
    ...
    Rprintf("bc         show current bytecode ( if available )\n");
    Rprintf("bcstack    show current bytecode stack ( if available )\n");
    Rprintf("Q          quit\n");
...
//            R_printCurrentBCbody(cntxt->bcbody, cntxt->bcpc, FALSE, 1);
        ...
        if(!R_BCIntActive) SET_RDEBUG(rho, 0); /*PR#1721*/
