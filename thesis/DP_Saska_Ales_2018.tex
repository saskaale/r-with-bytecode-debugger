...
The nice example of disassembler is in Java language~(\code{javap} command of Java package). The Java bytecode is although very specific and each file contains the one class. The whole file~(classfile) contains the representation of the file just as the bytecode. Although the GNU-R implementation is different - there can be mixed up the non-compiled~(AST) and compiled~(BC) code. It means that the bytecode printer is showing just the one function at once.
