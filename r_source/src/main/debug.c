...
/*
 * Setting flag if BC debugger should be verbose ( showing real BCode while debugging
 * 1 arguments,
 *      setTo ( TRUE / FALSE / NULL )
 * returns back previous value
 */
SEXP attribute_hidden do_debugVerbose(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    checkArity(op, args);

    Rboolean prev = R_DebugVerbose;

    if(length(CAR(args)) > 0)
        R_DebugVerbose = asLogical(CAR(args));

    return ScalarLogical(prev);
}

static Rboolean enable_bc_debug = FALSE;
#define GET_ENABLE_BC_DEBUG enable_bc_debug
#define SET_ENABLE_BC_DEBUG(value) enable_bc_debug = value

Rboolean R_is_bc_debug_enabled() { return GET_ENABLE_BC_DEBUG; }

SEXP attribute_hidden do_enableBCDebug(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    checkArity(op, args);
    Rboolean prev = GET_ENABLE_BC_DEBUG;

    /* can behave also just as getter */
    if(length(CAR(args)) > 0)
        SET_ENABLE_BC_DEBUG(asLogical(CAR(args)));
